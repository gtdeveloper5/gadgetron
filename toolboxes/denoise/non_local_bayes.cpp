...
#pragma omp parallel for num_threads(number_of_threads)
            ...
            hoNDArray<T> non_local_bayes_T(const hoNDArray<T> &image, float noise_std, unsigned int search_window) {
                ...
                for (int i = 0; i < n_images; i++) {
                    ...
                    hoNDArray<T> image_view;
                    image_view.create(image_dims, const_cast<T*>(image.get_data_ptr() + i * image_elements));

                    hoNDArray<T> result_view;
                    result_view.create(image_dims, const_cast<T*>(result.get_data_ptr() + i * image_elements));
