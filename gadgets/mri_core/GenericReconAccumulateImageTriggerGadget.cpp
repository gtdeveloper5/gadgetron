...
namespace Gadgetron { 
    ...
    {
        cha_trigger_ = trigger_channel.value();
        slc_trigger_ = trigger_slice.value();
        con_trigger_ = trigger_contrast.value();
        phs_trigger_ = trigger_phase.value();
        rep_trigger_ = trigger_repetition.value();
        set_trigger_ = trigger_set.value();
        ave_trigger_ = trigger_average.value();

        pass_image_immediate_ = pass_image_immediately.value();
        ...
        dimensions_.resize(num_of_dimensions_, 0);
        dimensions_[0] = 1;
                ...
                for (s = 0; s < S; s++)
                {
                    for (n = 0; n < N; n++)
                    {
                        for (cha = 0; cha < CHA; cha++)
                        {
                            ...
                            size_t N = imgBuf(0)->get_number_of_elements();
    ...
    int GenericReconAccumulateImageTriggerGadget<T, D>::trigger(ImageBufferType& buf, ImageSentFlagBufferType& sentFlagBuf, bool inClose)
    {
        try
        {
            ...
            // --------------------------------------------------------------------------------------------
            // loop over the un-triggered dimension
