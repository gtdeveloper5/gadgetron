...
namespace Gadgetron {
    ...
    int BucketToBufferGadget::process_config(ACE_Message_Block* mb)
    {
    ...
    int BucketToBufferGadget::process(GadgetContainerMessage<IsmrmrdAcquisitionBucket>* m1)
    {
    ...
    void BucketToBufferGadget::allocateDataArrays(IsmrmrdDataBuffered & dataBuffer, ISMRMRD::AcquisitionHeader & acqhdr, ISMRMRD::Encoding encoding, IsmrmrdAcquisitionBucketStats & stats, bool forref)
    {
        try
        {
            if (dataBuffer.data_.get_number_of_elements() == 0)
            {
                ...
                if (verbose.value())
                {
                    GDEBUG_STREAM("Data dimensions, for reference: " << forref << ", [NE0 NE1 NE2 NLOC NCHA NN NS]: [" << NE0 << " " << NE1 << " " << NE2 << " " << NLOC << " " << NCHA << " " << NN << " " << NS << "]");
                }
        ...
        {
            GADGET_CATCH_THROW("Exceptions happened in BucketToBufferGadget::allocateDataArrays(...) ... ");
        }
    }
    ...
    {
        // encoding space FOV and matrix size
        if (encoding.trajectory.compare("cartesian") == 0)
        {
        ...
        // recon space FOV and matrix size
        ...
        if ((encoding.trajectory.compare("cartesian") == 0) 
            && (!forref || (forref && (encoding.parallelImaging.is_present() 
                                        && encoding.parallelImaging.get().calibrationMode.is_present() 
                                        && encoding.parallelImaging.get().calibrationMode.get() == "embedded"))))
        {
            ...
            // enforce the "center alignment" constraint
            {
        ...
        if (verbose.value())
        {
            GDEBUG_STREAM("fillSamplingDescription for " << encoding.trajectory);
            GDEBUG_STREAM("sampling.encoded_FOV_            : " << sampling.encoded_FOV_[0] << " " << sampling.encoded_FOV_[1] << " " << sampling.encoded_FOV_[2]);
            GDEBUG_STREAM("sampling.encoded_matrix_         : " << sampling.encoded_matrix_[0] << " " << sampling.encoded_matrix_[1] << " " << sampling.encoded_matrix_[2]);
            GDEBUG_STREAM("sampling.recon_FOV_              : " << sampling.recon_FOV_[0] << " " << sampling.recon_FOV_[1] << " " << sampling.recon_FOV_[2]);
            GDEBUG_STREAM("sampling.recon_matrix_           : " << sampling.recon_matrix_[0] << " " << sampling.recon_matrix_[1] << " " << sampling.recon_matrix_[2]);
            GDEBUG_STREAM("sampling.sampling_limits_[0]     : " << sampling.sampling_limits_[0].min_ << " " << sampling.sampling_limits_[0].center_ << " " << sampling.sampling_limits_[0].max_);
            GDEBUG_STREAM("sampling.sampling_limits_[1]     : " << sampling.sampling_limits_[1].min_ << " " << sampling.sampling_limits_[1].center_ << " " << sampling.sampling_limits_[1].max_);
            GDEBUG_STREAM("sampling.sampling_limits_[2]     : " << sampling.sampling_limits_[2].min_ << " " << sampling.sampling_limits_[2].center_ << " " << sampling.sampling_limits_[2].max_);
        }
    }
    ...
    void BucketToBufferGadget::stuff(std::vector<IsmrmrdAcquisitionData>::iterator it, IsmrmrdDataBuffered & dataBuffer, ISMRMRD::Encoding encoding, bool forref)
    {
        try
        {
            ...
            // compute the data offset along RO
            ...
            // if carteisian sampling, filling in the data with "center alignment"
            ...
            // fill data, data header and trajctory
            std::complex<float>* dataptr = NULL;
        ...
        {
            GADGET_CATCH_THROW("Exceptions happened in BucketToBufferGadget::stuff(...) ... ");
        }
    }
